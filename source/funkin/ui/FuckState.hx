...
class FuckState extends FlxUIState {
	...
	@:keep inline public static function FUCK(e:Dynamic,?info:String = "unknown",_forced:Bool = false,_FATAL:Bool = false,_rawError:Bool=false){
		...
		if(lastERROR != exception){
			...
			try{
				...
				if(!FileSystem.exists('crashReports/')){
					FileSystem.createDirectory('crashReports/');
				}

				dateNow = StringTools.replace(StringTools.replace(_date.toString(), " ", "_"), ":", ".");
				try{ // Too many instances of this unironically throwing an error, fallback to normal one
					...
					err = '# Super vSlice backup Crash Reporter: \n# $funnyQuip\n${exception}\nThis happened in ${info}';
					try{
				...
				sys.io.File.saveContent('crashReports/SUPER-VSLICE_CRASH-${dateNow}.log',err);
				
				saved = true;
				trace('Wrote a crash report to ./crashReports/SUPER-VSLICE_CRASH-${dateNow}.log!');
				trace('Crash Report:\n$err');
			}catch(e){
		...
		if(Main.game == null || _rawError || useOpenFL){
			...
			try{Main.instance.removeChild(Main.game);}catch(e){};
			if(Main.game != null){
			...
			try{
				if(!showingError){
					...
					if(saved){
						...
						textFieldBot.text = 'Saved crashreport to "crashReports/SUPER-VSLICE_CRASH-${dateNow}.log".\nPlease send this file when reporting this crash.';
					}

					// textField.x = (1280 * 0.5);
					var tf = new TextFormat("VCR.ttf", 24, 0xFFFFFF);
					tf.align = "center";
	...
	override function create() {
		...
		if(saved) {
			...
			txt.text = 'Crash report saved to "crashReports/SUPER-VSLICE_CRASH-${dateNow}.log".\n ' + txt.text.substring(41);
		}
		useOpenFL = true;
	}
